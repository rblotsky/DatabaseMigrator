...
class ForeignKey(IMigratable):
    ...
    def to_dict(self):
        ...
        if self.localName != None:
            returnDict["local_name"] = self.localName
        if self.tableName != None:
            returnDict["table_name"] = self.tableName
        if self.externalName != None:
            returnDict["foreign_name"] = self.externalName
        if self.onDelete != None:
            returnDict["on_delete"] = self.onDelete
        if self.onUpdate != None:
            returnDict["on_update"] = self.onUpdate

        return returnDict

    def copy(self) -> "ForeignKey":
        copiedFKey = ForeignKey(
            self.localName,
            self.tableName,
            self.externalName,
            self.onDelete,
    ...
    def setup_references(self, ownTable: "Table", allTables: list["Table"]):
    ...
    def compare_contents(self, other: "ForeignKey") -> bool:
        return self.onDelete == other.onDelete and self.onUpdate == other.onUpdate

    def validate_self(self, tableUsed: "Table") -> list[ValidationError]:
        errors = []
        ...
        if self.onUpdate != None and not DataValidation.validate_fkey_constraint(
            self.onUpdate
        ):
            errors.append(
                ValidationError(
        ...
        if self.onDelete != None and not DataValidation.validate_fkey_constraint(
            self.onDelete
        ):
            errors.append(
                ValidationError(
        ...
        if self.localRef == None:
            errors.append(
                ValidationError(
                    ErrorType.UNKNOWN_NAME_REFERENCED,
...
class Column(IMigratable):
    ...
    def to_dict(self):
        ...
        if self.datatype != None:
            returnDict["type"] = self.datatype
        if self.constraints != None:
            returnDict["constraints"] = self.constraints
        return returnDict
    ...
    def copy(self) -> "Column":
        return Column(
            self.name,
            self.datatype,
            self.constraints.copy() if self.constraints != None else None,
        )
    ...
    def validate_self(self, tableUsed: "Table") -> list[ValidationError]:
        ...
        if self.datatype != None and not DataValidation.validate_datatype(
            self.datatype
        ):
            errors.append(
                ValidationError(
    ...
    def compare_contents(self, other: "Column") -> bool:
...
class Table(IMigratable):
    ...
    ## Initialization Functions
    def __init__(
        self, newName: str, newColumns: list[Column], newForeignKeys: list[ForeignKey]
    ):
        self.name = newName
    ...
    def setup_foreign_key_refs(self, allTables: list["Table"]):
    ...
    def compare_table_members(
        ownMembers: list[IMigratable], otherMembers: list[IMigratable]
    ) -> bool:

        # Short-circuit return if lists are differing lengths
        if len(ownMembers) != len(otherMembers):
            return False
        ...
        for i in range(len(own)):
            if not own[i].compare_equivalence(other[i]):
                return False

        return True

    def compare_contents(self, other: "Table") -> bool:
        return Table.compare_table_members(
    ...
    def copy(self) -> "Table":
        ...
        copiedColumns = (
            [col.copy() for col in self.columns] if self.columns != None else []
        )
        copiedFKeys = (
            [fKey.copy() for fKey in self.foreignKeys]
            if self.foreignKeys != None
            else []
        )
    ...
    def str_with_line_indicated(
        self,
        column: Column = None,
        foreignKey: ForeignKey = None,
        indicateSelf: bool = False,
    ):
...
class DatabaseSchema:
    ...
    def compare_equivalence(self, other: "DatabaseSchema"):
