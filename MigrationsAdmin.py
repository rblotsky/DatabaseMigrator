...
def open_schema_migration_files_from_folder(folder: Path) -> list[IOBase]:
    ...
    found_files = []
    for filePath in folder.glob(MIGRATIONS_FILE_REGEX):
...
def extract_schema_migrations_from_files(
    ...
    # Before returning, ensure none are missing
    foundMigrations.sort(key=lambda migration: migration.migrationIndex)

    highest_index = foundMigrations[-1].migrationIndex

    missing_migrations = get_missing_migration_indices(
        highest_index,
        [migration.migrationIndex for migration in foundMigrations],
    )
    if len(missing_migrations) > 0:
        raise ValueError(f"Missing migrations located: {missing_migrations}")
...
def open_sql_migration_files_from_folder(folder: Path) -> list[IOBase]:
    ...
    found_files = []
    for filePath in folder.glob(MIGRATIONS_SQL_FILE_REGEX):
...
def get_sql_migrations_as_dicts(opened_files: list[IOBase]) -> list[dict]:
    ...
    # Before returning, sort lowest to greatest and ensure none are missing
    foundMigrations.sort(key=lambda migration: migration["migrationIndex"])
    highest_index = foundMigrations[-1]["migrationIndex"]
    ...
    if len(missing_migrations) > 0:
        raise ValueError(f"Missing SQL migrations located: {missing_migrations}")

    return foundMigrations


def write_sql_migrations_combined_file(migrationsFolder: Path):

    # Opens all SQLMigration files
    if not os.path.exists(migrationsFolder):
        raise IOError(f"Migrations folder '{migrationsFolder}' does not exist!")
    ...
    combinedFile = open(
        os.path.join(migrationsFolder, SQL_MIGRATIONS_COMBINED_FILE), "w"
    )
...
def create_new_migration(dbSchemaFilePath: str, migrationsFolder: str):
    ...
    try:
        ...
        raise IOError(f"Failed to open file '{dbSchemaFilePath}': {err}")
    ...
    existingMigrations = extract_schema_migrations_from_files(
        open_schema_migration_files_from_folder(migrationsFolder)
    )
    ...
    newIndex = (
        existingMigrations[-1].migrationIndex + 1 if len(existingMigrations) > 0 else 0
    )
    ...
    if len(newMigration.tableMigrations) == 0:
        ...
        if ask_yes_no("Save this migration?"):
            ...
            newFile = open(
                os.path.join(migrationsFolder, create_migration_filename(newMigration)),
                "w",
            )
...
def test_migrations(migrationsFolder: str):
    ...
    print(
        pad_ok(
            f"Found migrations! Highest index: {existingMigrations[-1].migrationIndex}"
        )
    )
...
def create_sql_migrations(migrationsFolder: str):
    ...
    for migration in foundMigrations:
        ...
        if not os.path.exists(
            os.path.join(migrationsFolder, create_sqlmigration_filename(migration))
        ):
            print(
                pad_ok(
            ...
            createdSqlMigration = SQLMigrations.create_sql_for_schema_migration(
