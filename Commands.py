...
class Command:
    ...
    def try_call(self, *args):
        if len(args) != len(self.arguments):
            print(
                pad_err(
                    f"Expected {len(self.arguments)} arguments for command '{self.name}', got {len(args)}."
                )
            )
    ...
    def __str__(self):
        commandText = (
            f"Command '{self.name}': {self.desc} - Arguments: {self.arguments}"
        )
        return commandText


### MODULE FUNCTIONS ###
def assemble_command_dict(commands: list["Command"]) -> dict:
    newDict = {}
...
def try_call_command(commands: list["Command"], name: str, *args):
...
def get_command_list_text(commands: list["Command"]) -> str:
    outputText = ""
    for command in commands:
